...
shrink<- function(fileIn, fileOut, n, doCollapse =""){
    cmd<-paste("shrinkTable/shrinkTable.exe", fileIn, fileOut, n)
...
for(i in 1:length(percentages)) {
    ...
    write.csv(head(f1,n=n),makeFileName(nGramDirectory, ngramFile.1KNS))
    
    f1<-read.csv(makeFileName(nGramDirectory, ngramFile.1))
    write.csv(head(f1,n=n),makeFileName(nGramDirectory, ngramFile.1S))
